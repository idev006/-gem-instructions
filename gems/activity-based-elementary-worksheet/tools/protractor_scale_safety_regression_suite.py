...
add('profile-shape-integrity','perfect upper semicircle' in profile and 'non-uniform scaling' in profile)
...
add('w08-no-auto','final prompt must not contain unresolved `RENDER_PATH=AUTO`' in w08 and 'deterministic vector geometry' in w08)
...
add('w08-minimum-implies-65-reject','PRODUCTION_MIN_PROTRACTOR_WIDTH_MM=70' in w08)
...
compact=w09.replace(' ','')
add('w09-release-blocked','65mmrejected' in compact and 'PROMPT_RELEASE=BLOCKED' in w09)
