...
for skill,name in PACKS.items():
    ...
    # file check substitutes for first structural criterion; remaining seven plus exact ID = 8 total
    add(f'{skill}-id',f'SKILL_ID={skill}' in txt)
    for token in required_sections[1:]:
        add(f'{skill}-{token[:18]}',token in txt,token)

# 20 semantic/governance checks
...
# canonical math checks = remaining 5
h,m=3,30
