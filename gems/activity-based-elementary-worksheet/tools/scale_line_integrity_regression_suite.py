...
Protects scale/graduation semantics across supported instrument families.
Expected case count: exactly 40. Prompt-system QA only.
"""
...
('profile-minor-stroke','minor tick stroke width >=0.25 mm' in profile),
('profile-major-stroke','major tick stroke width >=0.35 mm' in profile),
('profile-major-length','major tick length >=1.5× minor length' in profile),
('profile-no-floating-ticks','No floating/detached tick fragments' in profile),
('profile-no-gray-required-lines','required marks cannot rely on gray-only strokes' in profile),
...
add('clock-60-positions','60 distinct minute positions' in clock and '6°' in clock)
add('dial-open-arc','50 active intervals' in dial and '60°' in dial and 'inactive' in dial.lower() and 'OPEN_ARC' in dial)
add('ruler-endpoint-inclusive','11 graduation positions' in ruler and 'cm marks stronger/longer than mm marks' in ruler)
add('thermometer-linear-scale','equal numeric intervals map to equal geometric spacing' in temp and 'labels align to major ticks' in temp)
...
add('builder-runs-scale-suite','scale_line_integrity_regression_suite.py' in builder and '1494/1494 PASS' in builder)
add('workflow-runs-scale-suite','Scale-line integrity regression' in workflow and '40 cases' in workflow)
add('validator-release-current','scale_line_integrity_regression_suite.py' in validator and '1494' in validator and '1494/1494 PASS' in checklist)
