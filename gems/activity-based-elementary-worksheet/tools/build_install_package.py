...
Package is derived from GitHub SSOT. Build is blocked unless both static SSOT
validation and the executable 449-case deterministic/policy dry-run pass.
"""
...
WORKER_BUNDLES: dict[str, list[str]] = {
    ...
    "W02_TIME_CLOCK.txt": [
        "workers/W02_TIME_CLOCK.md", "domains/TIME_ENGINE.md",
        "domains/CLOCK_READING_ENGINE.md", "domains/CLOCK_DAY_NIGHT_SINGLE_FACE_SPEC.md",
    ],
    ...
    "W05_TEMPERATURE_CAPACITY_VOLUME.txt": [
        ...
        "domains/TEMPERATURE_READING_ENGINE.md", "domains/CAPACITY_READING_ENGINE.md",
    ],
    "W06_MONEY_CALENDAR_DATA.txt": [
        "workers/W06_MONEY_CALENDAR_DATA.md", "domains/MONEY_ENGINE.md",
        "domains/CALENDAR_ENGINE.md", "domains/TABLE_GRAPH_READING_ENGINE.md",
    ],
    ...
    "W09_QA_RELEASE.txt": [
        "workers/W09_QA_RELEASE.md", "OUTPUT_CONTRACT.md", "ARCHITECTURE.md",
...
def bundle_text(worker_name: str, sources: list[str]) -> str:
    header = (
        ...
        f"RUNTIME KNOWLEDGE BUNDLE: {worker_name}\nBASELINE=2.6.x\nWORKER_SCHEMA_VERSION=1\n\n"
        "IMPORTANT RUNTIME PACKAGING NOTE\n"
        "Generated from GitHub SSOT. Embedded repository paths are provenance/logical references; "
        "supporting SSOT is included in this Knowledge bundle.\n\nEMBEDDED_SOURCES:\n"
    )
...
def main() -> int:
    ...
    dryrun = run_gate("full_dry_run_suite.py")
    if dryrun.returncode != 0:
        print(dryrun.stdout); print(dryrun.stderr, file=sys.stderr)
        print("BUILD BLOCKED: full dry-run suite failed", file=sys.stderr)
        return 1
    ...
    compact_note = (
        ...
        "This package was built only after SSOT validation and 449/449 deterministic/policy dry-run PASS.\n\n"
    )
    ...
    (guide_dir / "FULL_DRY_RUN_449_REPORT.txt").write_text(dryrun.stdout, encoding="utf-8")
